...
def render_player(self, language: str = 'vi'):
    ...
    if player.ping:
        txt += player.client.language.get(language, 'controller', 'music_server').format(host=player.node.label)
        txt += player.client.language.get(language, 'controller', 'discord_latency').format(endpoint=trim_text(endpoint, 10), ping=player.ping)
    ...
    # Text là chữ dưới chân embed, icon_url là cái icon nhỏ nhỏ bên cạnh cái chữ
    embed.set_footer(text="Music CitLaLi", icon_url="https://cdn.discordapp.com/attachments/1211567863538786334/1312988508645752892/ExuEyes.gif")
    ...
    view.add_item(Button(style=ButtonStyle.red, emoji="💌", custom_id="player_controller_stop_btn"))
